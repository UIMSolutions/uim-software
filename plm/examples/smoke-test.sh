...
echo "Running PLM smoke test against $BASE_URL for tenant $TENANT"
...
products=$(curl -sS "$BASE_URL/api/v1/plm/products") || fail "could not fetch products"
...
changes=$(curl -sS "$BASE_URL/api/v1/plm/change-requests") || fail "could not fetch change requests"
