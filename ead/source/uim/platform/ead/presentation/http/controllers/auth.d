module uim.platform.ead.presentation.http.controllers.auth;
...
struct AuthGuard {
    ...
    static bool ensureWrite(scope HTTPServerRequest req, scope HTTPServerResponse res, string objectType) {
        ...
        auto rolesHeader = req.headers.get("X-EAD-Roles", "");
        auto roles = parseRoles(rolesHeader);

        if (roles.canFind("ead.admin") || roles.canFind("ead.write") || roles.canFind(objectType ~ ".write")) {
            return true;
        }
