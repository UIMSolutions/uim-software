...
products=$(curl -sS "$BASE_URL/api/v1/epd/products") || fail "could not fetch products"
...
changes=$(curl -sS "$BASE_URL/api/v1/epd/change-requests") || fail "could not fetch change requests"
