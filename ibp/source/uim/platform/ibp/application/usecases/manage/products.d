module uim.platform.ibp.application.usecases.manage.products;
...
class ManageProductsUseCase : UIMUseCase {
    ...
    CommandResult create(ProductDTO dto) {
        ...
        if (!IbpValidator.isValidProduct(value)) return CommandResult(false, "", "Invalid product data");
        repo.save(value);
    ...
    CommandResult update(ProductDTO dto) {
        ...
        if (existing is null) return CommandResult(false, "", "Demand plan not found");
