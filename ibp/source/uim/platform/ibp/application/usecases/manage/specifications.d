module uim.platform.ibp.application.usecases.manage.specifications;
...
class ManageSpecificationsUseCase : UIMUseCase {
    ...
    CommandResult create(SpecificationDTO dto) {
        ...
        value.demandPlanId = dto.demandPlanId;
        value.name = dto.name;
        ...
        if (!EwmValidator.isValidSpecification(value)) {
    ...
    CommandResult update(SpecificationDTO dto) {
        ...
        if (existing is null) {
            return CommandResult(false, "", "Scenario simulation not found");
        }
