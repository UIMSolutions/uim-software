...
*****************************************************************************************************************/
module uim.platform.gtt.application.usecases.manage.manage_locations;
...
class ManageLocationsUseCase : UIMUseCase {
    ...
    Location[] listByCountry(string country) {
    ...
    CommandResult create(LocationDTO dto) {
        ...
        l.postalCode = dto.postalCode;
        l.latitude = dto.latitude;
        l.longitude = dto.longitude;
        l.timezone = dto.timezone;
        l.sourceSystem = dto.sourceSystem;
        l.createdBy = dto.createdBy;
        if (!TrackingValidator.isValidLocation(l))
    ...
    CommandResult update(LocationDTO dto) {
        ...
        if (dto.city.length > 0) existing.city = dto.city;
        if (dto.latitude.length > 0) existing.latitude = dto.latitude;
        if (dto.longitude.length > 0) existing.longitude = dto.longitude;
