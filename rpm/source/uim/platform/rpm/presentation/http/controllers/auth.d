module uim.platform.rpm.presentation.http.controllers.auth;
...
struct AuthGuard {
    ...
    static bool ensureWrite(scope HTTPServerRequest req, scope HTTPServerResponse res, string objectType) {
        ...
        auto rolesHeader = req.headers.get("X-RPM-Roles", "");
        auto roles = parseRoles(rolesHeader);

        if (roles.canFind("rpm.admin") || roles.canFind("rpm.write") || roles.canFind(objectType ~ ".write")) {
            return true;
        }
