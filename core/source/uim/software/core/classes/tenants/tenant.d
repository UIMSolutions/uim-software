...
*****************************************************************************************************************/
module uim.software.core.classes.tenants.tenant;
...
class UIMTenant : UIMEntity, IUIMTenant {
  ...
  override Json toJson() {
    auto j = super.toJson();
    j["id"] = Json(id.toString());
    j["name"] = Json(name);
    j["description"] = Json(description);
    j["domain"] = Json(domain);
    j["owner"] = Json(owner);
    j["created_at"] = Json(createdAt.toISOExtString());
    j["updated_at"] = Json(updatedAt.toISOExtString());
    j["settings"] = settings;
    return j;
  }

  override string toString() const {
    return "SAPTenant(id: " ~ _id.toString() ~ ", name: " ~ _name ~ ", domain: " ~ _domain ~ ", owner: " ~ _owner
      ~ ", createdAt: " ~ _createdAt.toString() ~ ", updatedAt: " ~ _updatedAt.toString() ~ ")";
  }
}
