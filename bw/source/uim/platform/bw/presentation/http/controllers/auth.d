module uim.platform.bw.presentation.http.controllers.auth;
...
struct AuthGuard {
    ...
    static bool ensureWrite(scope HTTPServerRequest req, scope HTTPServerResponse res, string objectType) {
        ...
        auto rolesHeader = req.headers.get("X-BW-Roles", "");
        auto roles = parseRoles(rolesHeader);

        if (roles.canFind("bw.admin") || roles.canFind("bw.write") || roles.canFind(objectType ~ ".write")) {
            return true;
        }
