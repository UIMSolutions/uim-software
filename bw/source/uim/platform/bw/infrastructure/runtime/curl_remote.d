module uim.platform.bw.infrastructure.runtime.curl_remote;
...
class CurlRemoteBwQueryRuntime : BwQueryRuntime {
    ...
    override Json execute(QueryExecutionDTO request) {
        ...
        meta["backend"] = Json(endpointUrl);
        meta["message"] = Json("Remote BW runtime call failed or returned non-JSON.");
        fallback["meta"] = meta;

        return fallback;
    }

    private Json toJson(QueryExecutionDTO request) {
        auto payload = Json.emptyObject;
