module uim.platform.ewm.application.usecases.integration.ewm_integrations;

import uim.platform.ewm;

@safe:

class RunEwmIntegrationsUseCase : UIMUseCase {
    ...
    CommandResult handoverProduct(ProductId id) {
        ...
        if (product is null) return CommandResult(false, "", "Warehouse not found");
    ...
    CommandResult syncSpecification(SpecificationId id) {
        ...
        if (specification is null) return CommandResult(false, "", "Stock item not found");
