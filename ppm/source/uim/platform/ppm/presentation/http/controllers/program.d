module uim.platform.ppm.presentation.http.controllers.program;

import std.conv : to;
import uim.platform.ppm;

@safe:

class ProgramController : SAPController {
    private ManageProgramsUseCase useCase;

    this(ManageProgramsUseCase useCase) { this.useCase = useCase; }

    override void registerRoutes(URLRouter router) {
        router.get("/api/v1/ppm/programs", &handleList);
        router.get("/api/v1/ppm/programs/*", &handleGet);
        router.post("/api/v1/ppm/programs", &handleCreate);
        router.put("/api/v1/ppm/programs/*", &handleUpdate);
        router.delete_("/api/v1/ppm/programs/*", &handleDelete);
    }

    private void handleList(scope HTTPServerRequest req, scope HTTPServerResponse res) {
        auto items = useCase.list();
        auto arr = Json.emptyArray;
        foreach (item; items) arr ~= programToJson(item);
        writeJsonBody(res, Json(["count": Json(cast(long) items.length), "resources": arr]));
    }

    private void handleGet(scope HTTPServerRequest req, scope HTTPServerResponse res) {
        auto id = extractIdFromPath(req.requestPath.to!string);
        auto item = useCase.get_(id);
        if (item is null) { writeError(res, 404, "Program not found"); return; }
        writeJsonBody(res, programToJson(*item));
    }

    private void handleCreate(scope HTTPServerRequest req, scope HTTPServerResponse res) {
        auto j = req.json;
        ProgramDTO dto;
        dto.id = jsonStr(j, "id");
        dto.tenantId = req.headers.get("X-Tenant-Id", "");
        dto.portfolioId = jsonStr(j, "portfolioId");
        dto.name = jsonStr(j, "name");
        dto.objective = jsonStr(j, "objective");
        dto.status = jsonStr(j, "status");
        dto.manager = jsonStr(j, "manager");
        dto.startDate = jsonStr(j, "startDate");
        dto.endDate = jsonStr(j, "endDate");
        dto.createdBy = jsonStr(j, "createdBy");
        auto result = useCase.create(dto);
        if (!result.success) { writeError(res, 400, result.error); return; }
        res.statusCode = cast(int) HTTPStatus.created;
        writeJsonBody(res, Json(["id": Json(result.id)]));
    }

    private void handleUpdate(scope HTTPServerRequest req, scope HTTPServerResponse res) {
        auto j = req.json;
        ProgramDTO dto;
        dto.id = extractIdFromPath(req.requestPath.to!string);
        dto.portfolioId = jsonStr(j, "portfolioId");
        dto.name = jsonStr(j, "name");
        dto.objective = jsonStr(j, "objective");
        dto.status = jsonStr(j, "status");
        dto.manager = jsonStr(j, "manager");
        dto.startDate = jsonStr(j, "startDate");
        dto.endDate = jsonStr(j, "endDate");
        dto.modifiedBy = jsonStr(j, "modifiedBy");
        auto result = useCase.update(dto);
        if (!result.success) { writeError(res, 404, result.error); return; }
        writeJsonBody(res, Json(["id": Json(result.id)]));
    }

    private void handleDelete(scope HTTPServerRequest req, scope HTTPServerResponse res) {
        auto id = extractIdFromPath(req.requestPath.to!string);
        auto result = useCase.remove(id);
        if (!result.success) { writeError(res, 404, result.error); return; }
        res.statusCode = cast(int) HTTPStatus.noContent;
        res.writeBody("");
    }
}
