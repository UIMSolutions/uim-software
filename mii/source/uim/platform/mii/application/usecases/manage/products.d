module uim.platform.mii.application.usecases.manage.products;
...
class ManageProductsUseCase : UIMUseCase {
    ...
    CommandResult create(ProductDTO dto) {
        ...
        if (!MiiValidator.isValidProduct(value)) return CommandResult(false, "", "Invalid product data");
        repo.save(value);
    ...
    CommandResult update(ProductDTO dto) {
        ...
        if (existing is null) return CommandResult(false, "", "Production message not found");
