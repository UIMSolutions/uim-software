...
*****************************************************************************************************************/
module uim.platform.itil.presentation.http.controllers.event_controller;
...
class EventController : SAPController {
    private ManageMonitoringEventsUseCase useCase;

    this(ManageMonitoringEventsUseCase useCase) { this.useCase = useCase; }

    void registerRoutes(URLRouter router) {
        router.get("/api/v1/itil/events",    &listAll);
        router.post("/api/v1/itil/events",   &create);
        router.get("/api/v1/itil/events/*",  &getOne);
        router.put("/api/v1/itil/events/*",  &update);
        router.delete_("/api/v1/itil/events/*", &remove);
    }

    private void listAll(HTTPServerRequest req, HTTPServerResponse res) @safe {
        auto items = useCase.list();
        auto arr = Json.emptyArray;
        foreach (e; items) arr ~= monitoringEventToJson(e);
    ...
    private void getOne(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        if (item is null) { writeError(res, 404, "Monitoring event not found"); return; }
        writeJsonBody(res, monitoringEventToJson(*item));
    }

    private void create(HTTPServerRequest req, HTTPServerResponse res) @safe {
        auto j = req.json;
        MonitoringEventDTO dto;
        dto.id               = jsonStr(j, "id");
        dto.tenantId         = jsonStr(j, "tenantId");
        dto.title            = jsonStr(j, "title");
        dto.description      = jsonStr(j, "description");
        dto.sourceCI         = jsonStr(j, "sourceCI");
        dto.affectedServiceId = jsonStr(j, "affectedServiceId");
        dto.detectedAt       = jsonStr(j, "detectedAt");
        dto.eventCode        = jsonStr(j, "eventCode");
        dto.eventSource      = jsonStr(j, "eventSource");
