...
*****************************************************************************************************************/
module uim.platform.itil.presentation.http.controllers.problem_controller;
...
class ProblemController : SAPController {
    private ManageProblemsUseCase useCase;

    this(ManageProblemsUseCase useCase) { this.useCase = useCase; }

    void registerRoutes(URLRouter router) {
        router.get("/api/v1/itil/problems",    &listAll);
        router.post("/api/v1/itil/problems",   &create);
        router.get("/api/v1/itil/problems/*",  &getOne);
        router.put("/api/v1/itil/problems/*",  &update);
        router.delete_("/api/v1/itil/problems/*", &remove);
    }

    private void listAll(HTTPServerRequest req, HTTPServerResponse res) @safe {
        auto items = useCase.list();
        auto arr = Json.emptyArray;
        foreach (p; items) arr ~= problemToJson(p);
    ...
    private void getOne(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        if (item is null) { writeError(res, 404, "Problem not found"); return; }
        writeJsonBody(res, problemToJson(*item));
    }
    ...
    private void create(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        dto.workaround       = jsonStr(j, "workaround");
    ...
    private void update(HTTPServerRequest req, HTTPServerResponse res) @safe {
        ...
        dto.rootCause   = jsonStr(j, "rootCause");
        dto.workaround  = jsonStr(j, "workaround");
        dto.solution    = jsonStr(j, "solution");
