...
*****************************************************************************************************************/
module uim.software.asset_performance.infrastructure.persistence.repositories.models;
...
class MemoryModelRepository : ModelRepository {
    ...
    @trusted Model* findById(ModelId id) {
        foreach (idx, ref m; store)
            if (m.id == id) return &store[idx];
        return null;
    }
