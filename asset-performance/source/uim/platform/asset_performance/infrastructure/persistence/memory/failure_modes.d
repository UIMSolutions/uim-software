...
*****************************************************************************************************************/
module uim.software.asset_performance.infrastructure.persistence.repositories.failure_modes;
...
class MemoryFailureModeRepository : FailureModeRepository {
    ...
    @trusted FailureMode* findById(FailureModeId id) {
        foreach (idx, ref fm; store)
            if (fm.id == id) return &store[idx];
        return null;
    }
    ...
    FailureMode[] findByModel(ModelId modelId) {
        FailureMode[] result;
        foreach (ref fm; store)
