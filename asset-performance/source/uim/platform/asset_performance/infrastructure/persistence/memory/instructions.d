...
*****************************************************************************************************************/
module uim.software.asset_performance.infrastructure.persistence.repositories.instructions;
...
class MemoryInstructionRepository : InstructionRepository {
    ...
    @trusted Instruction* findById(InstructionId id) {
        foreach (idx, ref i; store)
            if (i.id == id) return &store[idx];
        return null;
    }
