...
*****************************************************************************************************************/
module uim.software.asset_performance.infrastructure.persistence.repositories.equipment;
...
class MemoryEquipmentRepository : EquipmentRepository {
    ...
    @trusted Equipment* findById(EquipmentId id) {
        foreach (idx, ref e; store)
            if (e.id == id) return &store[idx];
        return null;
    }
    ...
    Equipment[] findByLocation(LocationId locationId) {
        Equipment[] result;
        foreach (ref e; store)
            if (e.locationId == locationId) result ~= e;
        return result;
    }
